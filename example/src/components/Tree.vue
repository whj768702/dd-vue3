...
export default {
  ...
  data() {
    return {
      ...
      children: [],
      droppedIn: null,
      localHttpOptions: this.httpOptions,
    };
  },
  mounted() {
  ...
  methods: {
    ...
    detectTarget(target, addOrRemove) {
      ...
      if (this.item.id_node !== 0) {
        ...
        if (this.item.id_type === 1) {
          ...
          while (target.classList&&target.classList[0] !== 'vddl-draggable')
    ...
    changeClass(target, addOrRemove) {
      if (addOrRemove) {
        target?.classList?.add&&target.classList.add(this.hoverClassName);
      } else
        target?.classList?.remove&&target.classList.remove(this.hoverClassName);
    },
    ...
    loadData(success = this.loadSuccess, failed = this.loadFailed) {
      if (this.isApiMode) {
        ...
        this.localHttpOptions['params'] = this.item;

        let url = this.apiUrl + (this.itemKey ? this.item[this.itemKey] : '');

        this.fetch(url, this.localHttpOptions).then(
            success,
            failed,
        ).catch(() => failed());
      } else {
    ...
    loadFailed(response) {
      console.log('loading failed: ', response);
    },
  },
};
